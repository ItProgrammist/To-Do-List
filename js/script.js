...
document.getElementById('task-description').style.maxWidth =
document.getElementById('tasks-body').offsetWidth * 0.95 + 'px';

// Saving #tasks-body div to the 'todo' key
...
function getTasks() {
  ...
  if (descendents.length == 0) return false;

  let status = descendents[0].parentElement.parentElement.
  style.textDecoration == 'line-through';
  console.log(status);

  if (descendents.length > 0) {
    res.push(
      `[{"name":"${descendents[0].innerHTML}", "is_completed":${status}}`
    );
  }

  for (let i = 1; i < descendents.length; ++i) {
    status = descendents[i].parentElement.parentElement.
    style.textDecoration == 'line-through';
    console.log(status);
    res.push(
      `{"name":"${descendents[i].innerHTML}", "is_completed":${status}}`
    );
  }
...
function save() {
  // Creating/filling 'todo' key
  localStorage.setItem(
    'todo', document.getElementById('tasks-body').innerHTML
  );
}
  ...
  for (let i = 0; i < buttons.length; ++i) {
    if (buttons[i].getAttribute('data-id') == 'edited') {
      buttons[i].onclick = function() {
        let text = buttons[i].parentElement.parentElement.
        children[0].children[0].innerHTML;
        text = prompt('Editing the task', text);

        if (text || text == '') {
          buttons[i].parentElement.parentElement.
          children[0].children[0].innerHTML = text;
        }
...
document.getElementById('import').onclick = function() {
  ...
  if (files.length <= 0) {
    to_load_file = true;
    alert("You haven't chosen the file yet!");
    return false;
  }

  if (to_load_file) {
    to_load_file = !(
      confirm('Are you sure you want to display the uploaded file?')
    );
    
    if (to_load_file) return false;
  }
